...
--
-- 대기열은 조건별 bucket으로 나뉘어 있고 참가자마다 조건이 다를 수 있다 (docs/07 §3.1).
-- 그래서 큐에서 빼는 key도 참가자마다 따로 받는다.
--
-- KEYS[1]                      qm:proposal:members:{proposalId}   SET
-- KEYS[2], KEYS[3], KEYS[4]    사용자 1의 active-proposal, active-request, queue bucket
-- KEYS[5], KEYS[6], KEYS[7]    사용자 2의 active-proposal, active-request, queue bucket
-- ...
--
-- ARGV[1]            proposalId
-- ARGV[2]            ttlSeconds
...
if #KEYS < 7 or (#KEYS - 1) % 3 ~= 0 then
  return redis.error_reply('BAD_KEYS: members, then 3 keys per user, 최소 2명')
end

local memberCount = (#KEYS - 1) / 3
...
for i = 1, memberCount do
  local proposalKey = KEYS[1 + (i - 1) * 3 + 1]
  local requestKey = KEYS[1 + (i - 1) * 3 + 2]
...
for i = 1, memberCount do
  local proposalKey = KEYS[1 + (i - 1) * 3 + 1]
  local bucketKey = KEYS[1 + (i - 1) * 3 + 3]
  local expectedRequestId = ARGV[2 + i * 2 - 1]
  local userId = ARGV[2 + i * 2]

  redis.call('SET', proposalKey, proposalId, 'EX', ttl)
  redis.call('ZREM', bucketKey, expectedRequestId)

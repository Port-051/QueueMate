...
test('활동 재확인과 조건 한 개 변경은 사용자 선택 후에만 적용된다', async ({ page }) => {
  ...
  await page.locator('.recruitment-composer-shell').getByLabel('주 포지션', { exact: true }).selectOption('TOP');
  await page.locator('.recruitment-composer-shell').getByRole('button', { name: '탑', exact: true }).click();
  ...
  await expect(page.locator('.recruitment-composer-shell').getByRole('button', { name: '탑', exact: true })).toHaveAttribute('aria-pressed', 'true');
  await page.keyboard.press('Escape');
  await page.getByRole('button', { name: /상대 포지션을 넓히면/ }).click();
  await page.getByRole('button', { name: '이 조건만 변경' }).click();
  await expect(page.getByRole('region', { name: '조건 변경 미리 보기' })).toHaveCount(0);
  await manageRecruitment(page, '조건 수정');
  await expect(page.locator('.recruitment-composer-shell').getByRole('button', { name: '탑', exact: true })).toHaveAttribute('aria-pressed', 'false');
  await expect(page.locator('.recruitment-composer-shell').getByLabel('주 포지션')).toHaveValue('TOP');
});

...
test('기본 랭크 모드로 선택 단계 없이 매칭을 시작한다', async ({ page }) => {
  ...
  await expect(dialog.getByRole('group', { name: '내 포지션', exact: true }).getByRole('button', { name: '무관', exact: true })).toHaveAttribute('aria-pressed', 'true');
...
test('검색과 별개로 아이콘 버튼을 선택하고 수정·다시 시작할 때 유지한다', async ({ page }) => {
  ...
  await form.getByRole('group', { name: '내 포지션', exact: true }).getByRole('button', { name: '미드', exact: true }).click();
  ...
  await expect(form.getByRole('group', { name: '내 포지션', exact: true }).getByRole('button', { name: '미드', exact: true })).toHaveAttribute('aria-pressed', 'true');

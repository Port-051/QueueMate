...
test('포지션 다중 선택은 합집합으로 검색하고 개별 해제와 초기화가 된다', async ({ page }) => {
  ...
  expect(await rows.locator('.row-roles > .recruitment-role-pair').evaluateAll(nodes => nodes.every(node => /내 포지션: (탑|정글|전체),/.test(node.getAttribute('aria-label') ?? '')))).toBe(true);
  await roles.getByRole('button', { name: '탑', exact: true }).click();

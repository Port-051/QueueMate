...
test('실시간 예시 모집은 30분과 하루 뒤에도 유지되지만 내 모집은 활동 확인이 필요하다', async ({ page }) => {
...
test('하루가 지나면 기본 예약 검색 시간과 예시 모집 시간이 함께 갱신된다', async ({ page }) => {
  await page.clock.install({ time: startTime });
  await login(page);
...
test('작성 중인 예약 입력은 시간이 지나도 보존하고 만료된 시간으로 제출하지 않는다', async ({ page }) => {
  ...
  await expect(dialog.getByLabel('모집 한마디')).toHaveValue('작성 중인 예약');
  await expect(dialog.getByRole('button', { name: '모집 시작', exact: true })).toBeDisabled();
  await page.keyboard.press('Escape');
  await expect(page.locator('.recruitment-row').first()).toBeVisible();
});

test('직접 등록한 예약은 원래 시간에 종료되고 다음 날 예시 모집으로 바뀌지 않는다', async ({ page }) => {
  await page.clock.install({ time: startTime });
  await login(page);
  await page.getByRole('tab', { name: '예약 매치', exact: true }).click();
  await page.locator('.intro-launch').getByRole('button', { name: '예약하기' }).click();

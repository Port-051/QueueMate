  ...
  for (const width of [390, 768, 1024, 1280]) {
    ...
    for (const route of ['home', 'me', 'friends', 'recent']) {
      ...
      for (const mode of ['실시간', '예약']) {
        ...
        await page.getByRole('button', { name: `+ ${mode} 모집 만들기` }).click();
...
test('모바일 메뉴로 이동하고 닉네임 변경 상태와 연결 해제 대상을 확인한다', async ({ page }) => {
  ...
  await page.getByRole('button', { name: 'VALORANT ID 등록', exact: true }).click();
  await expect(page.getByRole('dialog', { name: 'VALORANT ID 등록', exact: true })).toBeVisible();
  ...
  await expect(page.getByRole('button', { name: 'VALORANT ID 등록', exact: true })).toHaveCount(0);
  await expect(page.getByRole('button', { name: 'PUBG: BATTLEGROUNDS ID 등록', exact: true })).toBeVisible();

...
test('오케이를 취소해도 같은 모집과 타이머를 유지한다', async ({ page }) => {
  await page.clock.install(); await login(page); await startRealtimeMatch(page);
...
test('추천에 답하지 않아도 모집을 잠그지 않고 조건을 수정할 수 있다', async ({ page }) => {
  ...
  await manageRecruitment(page, '조건 수정');
  await expect(page.getByRole('button', { name: '모집 조건 저장', exact: true })).toBeEnabled();
...
test('조건 수정 중 새 후보가 생겨도 작성 내용을 덮어쓰지 않는다', async ({ page }) => {
  await page.clock.install(); await login(page); await startRealtimeMatch(page, true);
  await manageRecruitment(page, '조건 수정');
  await page.getByLabel('모집 한마디').fill('아직 저장하지 않은 모집 문구');
  await page.clock.fastForward(7000);
  await expect(page.getByLabel('모집 한마디')).toHaveValue('아직 저장하지 않은 모집 문구');
  await expect(page.getByRole('dialog')).toHaveCount(0);
  await page.getByRole('button', { name: '모집 조건 저장', exact: true }).click();
  await expect(page.locator('.my-recruitment')).toContainText('아직 저장하지 않은 모집 문구');
});

test('조건 수정 중 이전 오케이가 성사되면 종료된 모집 편집을 닫는다', async ({ page }) => {
  await page.clock.install(); await login(page); await startRealtimeMatch(page);
  await page.getByRole('button', { name: 'PlayMaker 모집 상세' }).click();
  await page.getByRole('button', { name: '같이 할래요' }).click();
  await manageRecruitment(page, '조건 수정');
  await page.getByLabel('모집 한마디').fill('매칭 전 작성 중');
  await page.clock.fastForward(22_000);

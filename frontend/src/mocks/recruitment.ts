...
const effectiveCondition = (r: BoardRow): MatchCondition => ({ ...r.condition, modeKey: selectedModes.get(r.id) ?? r.condition.modeKey });
const person = (r: BoardRow): BoardPerson => ({ id: r.id, userId: r.userId, nickname: r.nickname, condition: effectiveCondition(r), preferences: r.preferences });
function seed() {
  ...
  for (const game of ['LOL', 'VALORANT', 'PUBG'] as const) for (const type of ['REALTIME', 'RESERVATION'] as const) {
    ...
    for (const [modeIndex, mode] of config.modes.entries()) for (let i = 0; i < 10; i++) {
      ...
      const condition: MatchCondition = { game, modeKey: mode.modeKey, keyCondition: { type: config.keyConditionType, value: config.values[i % config.values.length] }, voicePreference: i % 4 === 0 ? 'NO_VOICE' : i % 4 === 1 ? 'REQUIRED' : 'OPTIONAL', playPurpose: i % 3 === 0 ? 'FUN' : 'RANK_UP' };
      const r: BoardRow = { id: uid(), userId: c.userId, nickname: modeIndex === 0 ? c.nickname : `${c.nickname} · ${mode.modeKey === 'ARAM' ? '칼바람' : mode.modeKey === 'SQUAD' ? '스쿼드' : '일반'}`, type, condition,
...
function view(row: BoardRow) { const current = refresh(row); return structuredClone({ ...current, condition: effectiveCondition(current), applicants: current.userId === db.me.id ? current.applicants : [], impressions: current.userId === db.me.id ? current.impressions : 0, alertEnabled: current.userId === db.me.id && current.alertEnabled }); }
...
function accepts(a: BoardPreferences, ca: MatchCondition, b: BoardPreferences, cb: MatchCondition) {
  ...
  return (!a.desiredKeys.length || cb.keyCondition.value === 'ANY' || a.desiredKeys.includes('ANY') || a.desiredKeys.includes(cb.keyCondition.value)) && (!a.purposeRequired || ca.playPurpose === cb.playPurpose);
}
function compatible(ca: MatchCondition, pa: BoardPreferences, cb: MatchCondition, pb: BoardPreferences) {
  ...
  if (modeOf(ca.game, ca.modeKey === 'ANY' ? cb.modeKey : ca.modeKey)?.roleUniqueness && ca.keyCondition.value !== 'ANY' && ca.keyCondition.value === cb.keyCondition.value) return false;
...
function matches(query: BoardSearch, row: BoardRow, viewerId = db.me.id) {
  ...
  if (query.browse) {
    ...
    if (query.condition.keyCondition.value !== 'ANY' && query.condition.keyCondition.value !== row.condition.keyCondition.value) return false;
...
function sourceCondition(condition: MatchCondition, modeKey = condition.modeKey): MatchCondition {
  ...
  return { ...condition, modeKey: modeKey === 'ANY' ? config.modes[0].modeKey : modeKey, keyCondition: { ...condition.keyCondition, value: condition.keyCondition.value === 'ANY' && !config.values.includes('ANY') ? config.values[0] : condition.keyCondition.value } };
}
function selectMode(row: BoardRow, modeKey: string) {
  ...
  row.members = row.members.map(member => ({ ...member, condition: { ...member.condition, modeKey: condition.modeKey } }));
}

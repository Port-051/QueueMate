...
import { timeLabel } from '../domain/recruitment';
...
export function RecruitmentClock({ row, now }: { row: BoardRow; now: number }) {
  ...
  return <div className="recruitment-clock"><span>{label}</span><strong role="timer" aria-label={label} aria-live="off">{formatElapsed(seconds)}</strong><small>{row.type === 'RESERVATION' && row.availableFrom && row.availableTo ? formatRange(row.availableFrom, row.availableTo) : `${timeLabel(row.createdAt)}에 시작`}</small></div>;
}

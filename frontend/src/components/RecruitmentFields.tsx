...
export function RecruitmentFields({ condition, preferences, onChange, compact = false, modeLocked = false }: {
  ...
  return <div className={compact ? 'recruitment-fields compact' : 'recruitment-fields'}>
    ...
    {hasRoles ? <label>포지션 / 역할<select aria-label="포지션 / 역할" value={condition.keyCondition.value} onChange={e => onChange({ ...condition, keyCondition: { ...condition.keyCondition, value: e.target.value } }, preferences)}>{keys.map(k => <option value={k.value} key={k.value}>{k.label}</option>)}</select></label> : null}
    ...
    {hasRoles ? <fieldset className="desired-keys"><legend>찾는 포지션 / 역할</legend><div className="chip-options"><button type="button" className={!preferences.desiredKeys.length ? 'chosen' : ''} aria-pressed={!preferences.desiredKeys.length} onClick={() => patch({ desiredKeys: [] })}>무관</button>{keys.filter(k => k.value !== 'ANY').map(k => <button type="button" key={k.value} aria-pressed={preferences.desiredKeys.includes(k.value)} className={preferences.desiredKeys.includes(k.value) ? 'chosen' : ''} onClick={() => patch({ desiredKeys: preferences.desiredKeys.includes(k.value) ? preferences.desiredKeys.filter(v => v !== k.value) : [...preferences.desiredKeys, k.value] })}>{k.label}</button>)}</div></fieldset> : null}

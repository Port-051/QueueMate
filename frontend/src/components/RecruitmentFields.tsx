...
import { keyConditionOptions, visibleModes, VOICE_OPTIONS, PURPOSE_OPTIONS } from '../domain/gameConfig';
...
export function RecruitmentFields({ condition, preferences, onChange, compact = false, modeLocked = false }: {
  ...
  return <div className={compact ? 'recruitment-fields compact' : 'recruitment-fields'}>
    <label>게임 모드<select aria-label="게임 모드" value={condition.modeKey} disabled={modeLocked} onChange={e => onChange({ ...condition, modeKey: e.target.value }, preferences)}>{visibleModes(condition.game).map(m => <option value={m.key} key={m.key}>{m.label}</option>)}</select></label>
    <label>내 포지션 / 역할<select aria-label="내 포지션 / 역할" value={condition.keyCondition.value} onChange={e => onChange({ ...condition, keyCondition: { ...condition.keyCondition, value: e.target.value } }, preferences)}>{keys.map(k => <option value={k.value} key={k.value}>{k.label}</option>)}</select></label>

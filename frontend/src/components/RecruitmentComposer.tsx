...
export function RecruitmentComposer({ initial, editing, onClose, onSaved }: {
  initial: board.BoardWrite; editing?: board.BoardRow; onClose: () => void; onSaved: (row: board.BoardRow) => Promise<void>;
}) {
  ...
  const timeError = value.type !== 'RESERVATION' ? '' : !value.availableFrom || !value.availableTo ? '예약 시간을 선택해 주세요.'
    ...
    : !editing && Date.parse(value.availableFrom) <= Date.now() ? '시작 시각은 현재 이후여야 합니다.' : '';

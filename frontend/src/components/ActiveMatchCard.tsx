...
import { IconBolt, IconClock, IconParty } from './icons';
...
export function ActiveMatchCard() {
  ...
  return <section className="home-active-match home-match-column" aria-labelledby="active-match-heading">
    ...
    {request ? <div className="match-live-card">
      {condition ? <HomeMatchHeading condition={condition} status={<span className="match-card-status is-live" role="status"><i aria-hidden="true" />{request.status === 'PROPOSED' ? '제안 도착' : '매칭 중'}</span>} /> : <h3>팀원을 찾고 있습니다</h3>}

...
export function RecruitmentDetail({ row, busy, hasSource, disabled, disabledReason, onClose, onJoin }: { row: BoardRow; busy: boolean; hasSource: boolean; disabled: boolean; disabledReason?: string; onClose: () => void; onJoin: () => void }) {
  return <Modal className="recruitment-detail" title="모집 상세" closeLabel="모집 상세 닫기" onClose={onClose}><div className="row"><Avatar name={row.nickname} /><div><b>{row.nickname}</b><p>{BOARD_STATUS[row.status]} · {row.members.length}/{row.targetSize}명</p></div></div><p>{row.description}</p><dl><dt>모집자 역할 → 찾는 상대</dt><dd>{keyConditionLabel(row.condition)} → {desiredLabel(row)}</dd><dt>상대 티어</dt><dd>{row.preferences.minTier ? TIER_LABELS[row.preferences.minTier] : '제한 없음'} ~ {row.preferences.maxTier ? TIER_LABELS[row.preferences.maxTier] : '제한 없음'}</dd><dt>음성 / 목적</dt><dd>{VOICE_LABEL[row.condition.voicePreference]} · {PURPOSE_LABEL[row.condition.playPurpose]}{row.preferences.purposeRequired ? ' 필수' : ' 선호'}</dd></dl>
    {row.type === 'RESERVATION' && row.availableFrom ? <p>{timeLabel(row.availableFrom)} ~ {row.availableTo ? timeLabel(row.availableTo) : ''}</p> : null}
    <p className="hint">{disabledReason ?? (row.status !== 'OPEN' ? '현재 참여 신청을 받지 않는 모집이에요. 다른 모집도 살펴보세요.' : '참여 전에 서로의 조건과 남은 자리를 다시 확인합니다.')}</p><Button variant="primary" block disabled={busy || disabled || row.status !== 'OPEN'} onClick={onJoin}>{hasSource ? '이 모집에 참여 신청' : '내 조건 입력하고 참여 신청'}</Button></Modal>;
}

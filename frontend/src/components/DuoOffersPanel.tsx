...
export function DuoOffersPanel({ source }: { source: BoardRow }) {
  ...
  return <section className="duo-offers" aria-label="함께할 상대">
    {sent.length ? <section className="duo-sent" aria-label="보낸 오케이"><h3>오케이 보냄 <span>{sent.length}</span></h3><p>상대의 응답을 기다리며 계속 매칭 중이에요.</p>{sent.map(offer => <div className="duo-sent-person" key={offer.id}><Avatar name={offer.peer.nickname} size={30} /><div><strong>{offer.peer.nickname}</strong><span>응답 기다리는 중</span></div><Button size="sm" variant="ghost" disabled={Boolean(busy)} aria-label={`${offer.peer.nickname} 오케이 취소`} onClick={() => void act(offer.id)}>취소</Button></div>)}</section> : null}
